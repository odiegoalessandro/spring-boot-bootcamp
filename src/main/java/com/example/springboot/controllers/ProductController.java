...
public class ProductController {
    ...
    public ResponseEntity<List<ProductModel>> getAllProducts(){
        ...
        if(!productsList.isEmpty()){
            for(ProductModel productModel : productsList){
                UUID id = productModel.getProductId();
    ...
    public ResponseEntity<Object> getProductById(@PathVariable(value="id") UUID id){
        Optional<ProductModel> products = productRepository.findById(id);

        return products.<ResponseEntity<Object>>map(
                productModel -> ResponseEntity.status(HttpStatus.OK).body(productModel))
                .orElseGet(() ->
                        ResponseEntity.status(HttpStatus.NOT_FOUND)
                                .body("Não encontramos nenhum produto com esse id"));
    }
